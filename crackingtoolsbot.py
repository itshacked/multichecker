...
async def Devsexpo(event):
    if event.sender_id != Config.OWNER_ID:
        ...
        if rip is False:
            await event.reply(
                "**To Use This Bot, Please Join My Channel. :) THIS BOT WAS MADE BY ARYA. FOR CONTACT @ARYA_ASSISTANCEBOT.**",
    ...
    if Counter <= 0:
        ...
        file1.write("\nCHECKED BY @MULTI_CHECKER_ROBOT. IF YOU WANT TO MAKE THIS TYPE OF BOT SO CONTACT ME ON  @ARYA_ASSISTANCEBOT. \n")
        file1.close()
        ok.delete()
...
Bot Made By @itshackedgiveaways**"""
...
**✅Send Screenshot To @Its_Hacked_Bot. Thanks For Using Me. 
Bot Made By @itshackedgiveaways**"""
        if meke.get("token"):
...
async def atomz(event):
    ...
    await event.reply(f'**Hai, {firstname} !, I Am Simple Checker Bot.\n PLease Type /help To See Cmds ! \nBy @itshackedgiveaways**')
